...
void __fastcall TForm1::MessageClick(TObject *Sender) {
	...
	bsSkinMessage1->MessageDlg("Encryptus", mtInformation,
...
void __fastcall TForm1::DeleteButtonClick(TObject *Sender) {
	if (ShellList->Focused()) {
		if (EmptyString.IsEmpty()) {
			...
			if (isfolder_ == true) {
				if ((msg->MessageDlg("Are you sure you want to delete " +
					nameOfItem + " folder?", mtConfirmation,
					TMsgDlgButtons() << mbYes << mbNo, 0)) == IDYES) {
			...
			label:
				if ((msg->MessageDlg("Are you sure you want to delete " +
					nameOfItem + " file?", mtConfirmation,
					TMsgDlgButtons() << mbYes << mbNo, 0)) == IDYES) {
...
void __fastcall TForm1::About1Click(TObject *Sender) {
	...
	BusinessSkinForm->ClientInActiveEffectType = bsieBlur;
	Aboutus->ShowModal();
}
// ---------------------------------------------------------------------------
...
void __fastcall TForm1::JamFileList1AddItem(TJamFileListItem *Item,
	bool &CanAdd)
...
void __fastcall TForm1::TotalItemsClick(TObject *Sender) {
	...
	msg->MessageDlg(totalcountmsg + countofitem, mtInformation,
		TMsgDlgButtons() << mbOK, 0); // set as counter UP limit for Memo
...
void __fastcall TForm1::EncryptClick(TObject *Sender) {
	...
	if ((algorithmSet == false) && ((passwordKey->Text.IsEmpty() == true) ||
		(passwordVerifyKey->Text.IsEmpty() == true)))
		msg->MessageDlg
			("Encryptus cannot continue.\nPlease select algorithm and fill in some keys to go.",
		mtInformation, TMsgDlgButtons() << mbOK, 0);
	else if (algorithmSet == false)
		msg->MessageDlg("Please select the appropriate algorithm.",
		mtInformation, TMsgDlgButtons() << mbOK, 0);
	else if ((passwordKey->Text.IsEmpty() == true) ||
		(passwordVerifyKey->Text.IsEmpty() == true))
		msg->MessageDlg("Cheating huh.\'\nPlease fill in some keys to go.",
		mtInformation, TMsgDlgButtons() << mbOK, 0);
	else if (compare != 0)
...
// ---------------------------------------------------------------------------
System::UnicodeString __fastcall TForm1::GetFileNameExtension
	(UnicodeString InputFileName, UnicodeString Extension) {
	UnicodeString copy_InputFile = InputFileName;
	return (copy_InputFile.Insert(Extension, InputFileName.Length() + 1));
}

System::UnicodeString __fastcall TForm1::RemoveFileExtension
	(UnicodeString InputFileName, UnicodeString Extension) {
...
System::UnicodeString __fastcall TForm1::GetExtension(UnicodeString InputFile,
	int indexInJamList) {
...
void __fastcall TForm1::DecryptClick(TObject *Sender) {
	...
	if ((algorithmSet == false) && ((passwordKey->Text.IsEmpty() == true) ||
		(passwordVerifyKey->Text.IsEmpty() == true)))
		msg->MessageDlg
			("Encryptus cannot continue.\nPlease select algorithm and fill in some keys for decryption.",
		mtInformation, TMsgDlgButtons() << mbOK, 0);
	else if (algorithmSet == false)
		msg->MessageDlg("Please select the appropriate algorithm.",
		mtInformation, TMsgDlgButtons() << mbOK, 0);
...
void __fastcall TForm1::CryptoProgress(TObject *Sender,
	TipcEzCryptProgressEventParams *e) {
...
void __fastcall TForm1::Zip1Progress(TObject *Sender,
	TipzZipProgressEventParams *e) {
	Form2->Progress->Value = e->PercentProcessed;
	Form2->Progress->Update();
}
// ---------------------------------------------------------------------------
